  ...
  return (
    <Card className="overflow-hidden border-primary/20 bg-primary/5 shadow-sm">
      <CardContent className={compact ? "p-5" : "p-5 sm:p-6"}>
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <div className="mt-2 flex items-end gap-3">
              <span className="text-4xl font-black leading-none text-foreground transition-all duration-700">{credits}</span>
              <span className="mb-1 rounded-full border border-primary/30 bg-background px-3 py-1 text-xs font-bold text-primary shadow-[0_0_18px_hsl(var(--primary)/0.12)]">
                {tier.name}
              </span>
            </div>
          </div>
        ...
        <Button className="mt-5 w-full gap-2" onClick={() => navigate("/referrals")}>
