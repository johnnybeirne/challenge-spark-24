...
const GrowthToFeaturedBar = ({ points, compact }: { points: number; compact?: boolean }) => {
  const navigate = useNavigate();
  const span = Math.max(1, FEATURED_MIN - GROWTH_MIN);
  const pct = Math.min(100, Math.max(0, ((points - GROWTH_MIN) / span) * 100));
...
const PointStatusCard = ({ points, compact = false }: PointStatusCardProps) => {
  const navigate = useNavigate();
  ...
  return (
    <Card className="overflow-hidden border-primary/20 bg-primary/5 shadow-sm">
      <CardContent className={compact ? "p-4" : "p-5 sm:p-6"}>
        <GrowthToFeaturedBar points={points} compact={compact} />
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <div className={compact ? "mt-2 flex items-end gap-2" : "mt-2 flex items-end gap-3"}>
              <span className={compact ? "text-3xl font-black leading-none text-foreground transition-all duration-700" : "text-4xl font-black leading-none text-foreground transition-all duration-700"}>{points}</span>
        ...
        {!compact && <div className="mt-5 rounded-xl border border-border bg-background/70 p-4">
          ...
          <p className="mt-1 text-sm font-semibold text-foreground">
            {nextReward ? `${nextReward.points} Points — ${nextReward.title}` : "All Phase 1 unlocks are available"}
          </p>
        </div>}
