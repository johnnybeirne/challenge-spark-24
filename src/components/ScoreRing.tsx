...
/**
 * ScoreRing — one combined circular ring showing the overall score, split into
 * three equal-arc segments in the app's System / Audience / Conversion brand
 * colours, in fixed clockwise order System -> Audience -> Conversion.
 *
 * Each segment gets a small pill label floating just outside its arc with the
 * category name and percentage. The centre shows the overall score, a thin
 * divider, and "OUT OF 100" in small-caps tracked style.
 *
 * Two modes:
...
interface ScoreRingProps {
  ...
  segments: [ScoreRingSegment, ScoreRingSegment, ScoreRingSegment];
  /** Overall score shown in the centre, 0-100. */
  overall: number;
  /** Sweep + count-up animation on mount. Defaults to false (static). */
  animated?: boolean;
  /** Optional extra content rendered in the centre under "OUT OF 100"
   *  (e.g. the archetype pill on the results page). */
...
// Segment start angles (SVG degrees, 0 = 3 o'clock, clockwise).
...
const ScoreRing = ({
  ...
  useEffect(() => {
    ...
    // Kick off the arc sweep on the next frame so the transition plays.
