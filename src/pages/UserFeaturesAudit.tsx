...
const CONFLICTS = [
  { flag: "Premium modules 4 & 5 (Advanced Systems, Scaling With LeadTree) are stub only", severity: "warn" },
...
const CORE_ENTRY_LINKS: CoreEntryLink[] = [
  ...
  { title: "Premium / VIP Course", route: "/premium", description: "Premium landing (LeadTree Growth Accelerator) with coupon (FOUNDING497), pricing, modules. Primary CTA: 'Invite Others' / 'Become a Partner'.", badge: "Premium" },
    ...
    <Card className="border-border">
      <CardContent className="p-5">
        ...
        <p className="mb-4 text-sm text-muted-foreground">
          Primary URLs to test, promote, and manage the main LeadTree funnels.
        </p>
...
const NAV_COHESION_CHECKS: NavCheck[] = [
  ...
  { label: "ChallengeOS naming retired (UI + utilities)", status: "Detected", note: "Renamed to LeadTree across calendar, reset-password, etc." },
...
const RECOMMENDATIONS = [
  ...
  "Ship Premium modules 4 & 5 (Advanced Systems, Scaling With LeadTree) — currently stub only.",
...
const NOT_SHIPPED: NotShippedItem[] = [
  { name: "Premium training modules 4 & 5 (Advanced Systems, Scaling With LeadTree)", area: "Training", state: "Stub only", note: "Sidebar entries exist; lessons gated behind isPremiumUser with no real content." },
...
const UserFeaturesAudit = () => {
  ...
  const buildSummary = () => {
    const lines: string[] = [];
    lines.push(`LeadTree User Journey & Feature Audit — ${auditedAt.toLocaleString()}`);
    lines.push("");
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-8">
      ...
      <header className="mb-6">
        <h1 className="text-3xl font-black sm:text-4xl">LeadTree User Journey & Feature Audit</h1>
