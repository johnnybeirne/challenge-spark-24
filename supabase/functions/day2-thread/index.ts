...
async function handleInsight(payload: { key?: string; label?: string; inputs?: Day1Inputs }): Promise<Response> {
  ...
  const prompt = [
    ...
    "Constraints:",
    "- Write in short paragraphs. Each paragraph must contain exactly one sentence. Leave a blank line between every paragraph. Do not combine multiple points into one paragraph.",
    "- Two to four short paragraphs total. Each paragraph must be exactly one sentence.",
