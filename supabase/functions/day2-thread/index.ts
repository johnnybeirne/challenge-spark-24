...
async function handleInsight(payload: { key?: string; label?: string; inputs?: Day1Inputs }): Promise<Response> {
  ...
  const prompt = [
    ...
    "Constraints:",
    "- Write in short paragraphs. Never write more than 2 sentences before starting a new paragraph. Leave a blank line between every paragraph. Do not combine multiple points into one paragraph.",
    "- Two to four short paragraphs total. Each paragraph must be 1 or 2 sentences only.",
